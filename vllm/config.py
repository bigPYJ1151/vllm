...
class ModelConfig:
    ...
    def verify_with_parallel_config(
        ...
        if total_num_attention_heads % tensor_parallel_size != 0:
            ...
            setattr(self.hf_text_config,
                    "head_dim", head_dim)
            setattr(self.hf_config,
                    "num_key_value_heads", round_to_size(total_kv_heads, tp_size))
            setattr(self.hf_config,
                    "num_attention_heads", round_to_size(total_kv_heads, tp_size) * query_heads_per_kv)
            setattr(self.hf_text_config,
                    "num_attention_heads", round_to_size(total_kv_heads, tp_size) * query_heads_per_kv)
            if hasattr(self.hf_config, "intermediate_size"):
                intermediate_size = getattr(self.hf_config, "intermediate_size")
                setattr(self.hf_config,
                        "intermediate_size", round_to_size(intermediate_size, tp_size * 8))
