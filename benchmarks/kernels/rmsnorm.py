...
class RMSNormBench(KernelBenchmark):
      def __init__(self, loop_time, token_num, hidden_size, dtype:torch.dtype, device: torch.device) -> None:
          super().__init__(loop_time)
          self.x = torch.randn(token_num, hidden_size, dtype=dtype, device=device)
