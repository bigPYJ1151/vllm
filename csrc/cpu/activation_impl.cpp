...
void gelu_and_mul_cpu(torch::Tensor &out,   // [..., d]
...
void gelu_tanh_and_mul_cpu(torch::Tensor &out,   // [..., d]
                           torch::Tensor &input) // [..., 2 * d]
{
  int num_tokens = input.numel() / input.size(-1);
  int d = input.size(-1) / 2;
  ...
  VLLM_DISPATCH_FLOATING_TYPES(
      input.scalar_type(), "gelu_tanh_and_mul_cpu_impl", [&] {
        CPU_KERNEL_GUARD_IN(gelu_tanh_and_mul_cpu_impl)
        activation_kernel<scalar_t, gelu_tanh_act, true>(
            num_tokens, d, input.data_ptr<scalar_t>(),
            out.data_ptr<scalar_t>());
        CPU_KERNEL_GUARD_OUT(gelu_tanh_and_mul_cpu_impl)
      });
}
...
void gelu_new_cpu(torch::Tensor &out, torch::Tensor &input) {
  ...
  VLLM_DISPATCH_FLOATING_TYPES(input.scalar_type(), "gelu_new_cpu_impl", [&] {
...
void gelu_fast_cpu(torch::Tensor &out, torch::Tensor &input) {
  ...
  VLLM_DISPATCH_FLOATING_TYPES(input.scalar_type(), "gelu_fast_cpu_impl", [&] {
    CPU_KERNEL_GUARD_IN(gelu_fast_cpu_impl)
    activation_kernel<scalar_t, gelu_fast_act, false>(
        num_tokens, d, input.data_ptr<scalar_t>(), out.data_ptr<scalar_t>());
    CPU_KERNEL_GUARD_OUT(gelu_fast_cpu_impl)
  });
}
