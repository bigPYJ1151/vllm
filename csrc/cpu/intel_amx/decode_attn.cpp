...
void set_kv_buffer(
  ...
  int64_t num_tokens = loc.size(0);
  int64_t num_heads_kv = k_buffer.size(1);
  int64_t head_size = k_buffer.size(2);
  int64_t head_size_v = v_buffer.size(2);
  ...
  // check index data types
  const auto index_dtype = loc.scalar_type();
  TORCH_CHECK(index_dtype == at::kInt)

  AT_DISPATCH_REDUCED_FLOATING_TYPES(key.scalar_type(), "set_kv_buffer_kernel", [&] {
    AT_DISPATCH_INDEX_TYPES(index_dtype, "set_kv_buffer_indices", [&] {
