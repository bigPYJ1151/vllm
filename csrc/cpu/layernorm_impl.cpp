...
template <typename scalar_t>
void fused_add_rms_norm_cpu_impl(scalar_t *__restrict__ input,
...
void rms_norm_cpu(torch::Tensor &out, torch::Tensor &input,
  ...
  VLLM_DISPATCH_FLOATING_TYPES(input.scalar_type(), "rms_norm_cpu_impl", [&] {
    CPU_KERNEL_GUARD_IN(rms_norm_cpu_impl)
    rms_norm_cpu_impl(out.data_ptr<scalar_t>(), input.data_ptr<scalar_t>(),
                      weight.data_ptr<scalar_t>(), epsilon, num_tokens,
                      hidden_size);
    CPU_KERNEL_GUARD_OUT(rms_norm_cpu_impl)
  });
}
