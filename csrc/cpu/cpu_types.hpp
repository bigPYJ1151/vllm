...
template <typename scalar_t, int elem_num>
struct Vec {
  constexpr static int VEC_ELEM_NUM = elem_num;
  using vec_scalar_t = scalar_t;

  constexpr static int get_elem_num() { return VEC_ELEM_NUM; }
};

template <int elem_num>
struct FP32Vec : public Vec<float, elem_num> {};

template <int elem_num>
struct BF16Vec : public Vec<c10::BFloat16, elem_num> {};
...
struct BF16Vec8 : public BF16Vec<8> {
  __m128i reg;
...
struct BF16Vec16 : public BF16Vec<16> {
...
struct BF16Vec32 : public BF16Vec<32> {
  __m512i reg;
  ...
  explicit BF16Vec32(BF16Vec8& vec8_data)
      : reg(_mm512_broadcast_i32x4(vec8_data.reg)) {}

  void save(void* ptr) const { *reinterpret_cast<__m512i*>(ptr) = reg; }
};

struct FP32Vec4 : public FP32Vec<4> {
  union AliasReg {
    __m128 reg;
...
struct FP32Vec8 : public FP32Vec<8> {
  union AliasReg {
    __m256 reg;
    float values[VEC_ELEM_NUM];
  };
...
struct FP32Vec16 : public FP32Vec<16> {
  union AliasReg {
    __m512 reg;
    float values[VEC_ELEM_NUM];
  };
  ...
  explicit FP32Vec16(const FP32Vec4& data)
      : reg(_mm512_broadcast_f32x4(data.reg)) {}
