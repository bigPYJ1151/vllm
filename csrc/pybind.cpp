...
void paged_attention_v1_dispatch(
  ...
  VLLM_DISPATCH_DEVICES(out.device(), paged_attention_v1, out, query, key_cache, value_cache, head_mapping, scale, block_tables, context_lens, block_size, max_context_len, alibi_slopes);
}
...
void paged_attention_v2_dispatch(torch::Tensor &out, torch::Tensor &exp_sums,
    ...
    torch::Tensor &key_cache, torch::Tensor &value_cache, torch::Tensor &head_mapping, 
    float scale, torch::Tensor &block_tables, torch::Tensor &context_lens, int block_size,
    int max_context_len, const c10::optional<torch::Tensor> &alibi_slopes) {
  VLLM_DISPATCH_DEVICES(out.device(), paged_attention_v2, out, exp_sums, max_logits, tmp_out, query, key_cache, value_cache, head_mapping, scale, block_tables, context_lens, block_size,max_context_len, alibi_slopes);
}
