...
template<typename T>
__inline__ T silu_cpu(const T& x) {
  // x * sigmoid(x)
  return (T) (((float) x) / (1.0f + expf((float) -x)));
}

template <typename scalar_t>
void silu_and_mul_cpu_impl(int num_tokens, int d, scalar_t *__restrict__  input, scalar_t *__restrict__  output) {
...
void silu_and_mul_cpu(torch::Tensor &out, torch::Tensor &input) {
  TORCH_CHECK(input.scalar_type() == c10::ScalarType::Float);
    ...
    AT_DISPATCH_FLOATING_TYPES(input.scalar_type(), "rotary_embedding_neox_impl", [&] {silu_and_mul_cpu_impl(
      num_tokens,
      d,
