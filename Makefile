...
VLLM_TP_bench:
	cd benchmarks && \
	 VLLM_CPU_OMP_THREADS_BIND="0-15,24-39" \
...
VLLM_LT_bench:
	OMP_DISPLAY_ENV=verbose \
	 ...
	 TORCH_LOGS="recompiles" \
	 LD_PRELOAD=/usr/lib/x86_64-linux-gnu/libtcmalloc_minimal.so.4 \
	cd benchmarks && python3 -m cProfile -o small benchmark_latency.py --model=Felladrin/Llama-68M-Chat-v1 --n=1 --batch-size=128 --input-len=128 --output-len=256 --num-iters-warmup=1 --num-iters=3 --dtype=bfloat16 --trust-remote-code --device=cpu
...
VLLM_bench_client:
	cd benchmarks && python3 benchmark_serving.py --backend vllm --model lmsys/vicuna-7b-v1.5 --tokenizer lmsys/vicuna-7b-v1.5 --dataset ./ShareGPT_V3_unfiltered_cleaned_split.json --request-rate 1 --num-prompts 1000